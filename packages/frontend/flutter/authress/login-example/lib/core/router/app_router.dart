    ...
    routes: [
      ...
      GoRoute(
        ...
        redirect: (context, state) {
          ...
          return AuthressRouteGuard.roleGuard(context, state, requiredRoles: ['admin'], redirectTo: '/unauthorized');
        },
      ...
      // Error pages
      GoRoute(path: '/unauthorized', name: 'unauthorized', builder: (context, state) => const UnauthorizedPage()),

      GoRoute(path: '/error', name: 'error', builder: (context, state) => const ErrorPage()),

      // Loading page (useful for deep link handling)
      GoRoute(path: '/loading', name: 'loading', builder: (context, state) => const LoadingPage()),
    ],
...
class AdminPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Admin Panel'), backgroundColor: Colors.red.shade100),
      body: const Center(
        child: Column(
...
class ManagerPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Manager Dashboard'), backgroundColor: Colors.blue.shade100),
      body: const Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            ...
            SizedBox(height: 16),
            Text('Manager Dashboard', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
            SizedBox(height: 8),
...
class UnauthorizedPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Access Denied'), backgroundColor: Colors.orange.shade100),
      body: Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
