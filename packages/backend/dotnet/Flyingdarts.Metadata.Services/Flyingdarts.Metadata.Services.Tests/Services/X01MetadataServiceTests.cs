...
public class X01MetadataServiceTests
{
    ...
    public async Task GameTheory(int sets, int legs)
    {
        ...
        while (player1Sets < setsToWinGame && player2Sets < setsToWinGame)
        {
            ...
            var result = await _service.GetMetadataAsync(gameId, CancellationToken.None);
    ...
    public async Task GetMetadata_WithBestOf5Legs3Sets_CompleteGame_ReturnsCorrectMetadata()
    {
        ...
        SetupMockDynamoDbService(game, players, users, darts);
        var initialResult = await _service.GetMetadataAsync(gameId, CancellationToken.None);
        var set1Starter = initialResult.NextPlayer ?? throw new InvalidOperationException("NextPlayer should not be null for a game with no darts");
        ...
        SetupMockDynamoDbService(game, players, users, darts);
        result = await _service.GetMetadataAsync(gameId, CancellationToken.None);
    ...
    {
        var game = Game.Create(2, X01GameSettings.Create(3, 3));
        game.GameId = gameId;
    ...
    private List<GamePlayer> CreateTestPlayers(long gameId)
    {
        ...
        {
            GamePlayer.Create(gameId, "player1"),
            GamePlayer.Create(gameId, "player2")
        };
    }

    private List<User> CreateTestUsers(List<GamePlayer> players)
    {
        return players.Select(p => new User
        {
            UserId = p.PlayerId,
    ...
    {
        _mockDynamoDbService.Setup(x => x.ReadGameAsync(It.IsAny<long>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<Game> { game });
        
        _mockDynamoDbService.Setup(x => x.ReadGamePlayersAsync(It.IsAny<long>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(players);
        
        _mockDynamoDbService.Setup(x => x.ReadGameDartsAsync(It.IsAny<long>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(darts);
        
        _mockDynamoDbService.Setup(x => x.ReadUsersAsync(It.IsAny<string[]>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(users);
    }
    ...
    private void AssertGameState(Flyingdarts.Metadata.Services.Dtos.Metadata result, string player1Sets, string player1Legs, string player2Sets, string player2Legs, string? nextPlayer, string? winningPlayer)
    {
