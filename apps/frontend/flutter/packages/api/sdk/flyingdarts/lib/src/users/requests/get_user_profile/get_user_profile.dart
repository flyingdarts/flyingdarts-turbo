...
extension GetUserProfile on UsersApi {
  Future<RequestResponse<GetUserProfileSuccessResponse, RequestFailedResponse>> getUser(String userId) async {
    Uri uri = UsersEndpoints.applyPathSections(config.rootUri, UsersEndpoints.profile);
    Map<String, dynamic> queryParameters = {'userId': userId};

    try {
      Options options = OptionsBuilder(config).useAuthorizationToken().build();

      var res = await dio.get(uri.toString(), queryParameters: queryParameters, options: options);
      ...
      try {
        ...
        return SuccessResponse(GetUserProfileSuccessResponse(res.statusCode!, userProfile));
      } catch (err) {
        throw InvalidFormatException(err, "GET", uri.toString());
      }
    } on DioError catch (err) {
      var response = responseHandler.handleErrorResponse<GetUserProfileSuccessResponse>(err);
      return response;
    }
  }
}
