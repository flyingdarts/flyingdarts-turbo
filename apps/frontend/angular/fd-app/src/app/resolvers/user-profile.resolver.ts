...
export class SessionUserResolver implements Resolve<Promise<boolean>> {
  ...
  async resolve(): Promise<boolean> {
    ...
    try {
      const idTokenRaw = localStorage.getItem(
        "AuthenticationCredentialsStorage"
      );
      if (!idTokenRaw)
        throw new Error("No Authress credentials found in localStorage");
  ...
  private createUserProfileFromToken(token: string): UserProfileDetails {
    console.log(
      "[SessionUserResolver] Creating user profile from Authress token"
    );
    try {
      ...
      console.log(
        `[SessionUserResolver] Extracted name: ${userName}, email: ${email}, picture: ${picture}, userId: ${userId}`
      );
      return {
        UserName: userName,
        Email: email,
      ...
      console.error(
        `[SessionUserResolver] Error creating user profile from token: ${error}`
      );
      throw error;
    }
  }
