...
export class GameContainerComponent implements AfterViewInit {
  ...
  constructor(
    ...
    this.sdkService.instance?.metadata$.subscribe((metadata) => {
      console.log(
        "[GameContainer] Metadata received:",
    ...
    this.store
      ...
      .pipe(
        ...
        map(([prevState, currState]) => {
          console.log(
            "[GameContainer] Previous state:",
            JSON.stringify(prevState, null, 2)
          );
          console.log(
            "[GameContainer] Current state:",
            JSON.stringify(currState, null, 2)
          );
    ...
    this.isPlayerTurn$ = this.store
      .select(GameStateSelectors.selectNextPlayer)
      .pipe(map((nextPlayer) => nextPlayer === this.userId));
  ...
  private handleMetadata(metadata: Metadata) {
    ...
    const playerDarts: DartDto[] =
      Darts && player?.PlayerId ? Darts[player.PlayerId] || [] : [];
    const opponentDarts: DartDto[] =
      Darts && opponent?.PlayerId ? Darts[opponent.PlayerId] || [] : [];
    ...
    console.log(
      "[GameContainer] Parsed game state:",
      JSON.stringify(gameState, null, 2)
    );
    this.store.dispatch(GameStateActions.updateState({ state: gameState }));
    console.log(
      "[GameContainer] Dispatched updateState with:",
      JSON.stringify(gameState, null, 2)
    );
  }
