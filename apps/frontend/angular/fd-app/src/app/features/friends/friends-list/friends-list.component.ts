...
import {
  FriendsStateActions,
  FriendsStateSelectors,
} from "src/app/state/friends";
...
export class FriendsListComponent {
  ...
  constructor(private router: Router, private store: Store) {
    this.friends$ = this.store.select(
      FriendsStateSelectors.selectFriendsStateFriends
    );
  }
  ngOnInit(): void {
  ...
  removeFriend(friend: FriendDto): void {
    if (
      ...
      this.store.dispatch(
        FriendsStateActions.removeFriend({ friendId: friend.UserId })
      );
    }
  }
}
