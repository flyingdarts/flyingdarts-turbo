...
export class FriendRequestsComponent {
  ...
  constructor(private friendsService: FriendsService, private store: Store) {
    this.friendRequests$ = this.store.select(
      FriendsStateSelectors.selectFriendsStateFriendRequests
    );

    this.incomingRequests$ = this.friendRequests$.pipe(
      map((requests) => requests?.IncomingRequests ?? [])
    );

    this.outgoingRequests$ = this.friendRequests$.pipe(
      map((requests) => requests?.OutgoingRequests ?? [])
    );

    this.hasIncomingRequests$ = this.incomingRequests$.pipe(
      map((requests) => requests.length > 0)
    );

    this.hasOutgoingRequests$ = this.outgoingRequests$.pipe(
      map((requests) => requests.length > 0)
    );

    this.hasAnyRequests$ = combineLatest([
      this.hasIncomingRequests$,
      this.hasOutgoingRequests$,
    ]).pipe(map(([hasIncoming, hasOutgoing]) => hasIncoming || hasOutgoing));
  }
