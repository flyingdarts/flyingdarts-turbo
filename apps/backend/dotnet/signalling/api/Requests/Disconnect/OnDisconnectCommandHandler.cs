    ...
    : IRequestHandler<OnDisconnectCommand, APIGatewayProxyResponse>
{
    ...
    {
        Console.WriteLine(
            $"[OnDisconnect] Starting disconnect process for ConnectionId: {request.ConnectionId}"
        );
        ...
        try
        {
            ...
            {
                Console.WriteLine(
                    $"[OnDisconnect] Found user for ConnectionId: {request.ConnectionId}, UserId: {user.UserId}"
                );
                ...
                Console.WriteLine(
                    $"[OnDisconnect] Successfully removed connection ID for UserId: {user.UserId}"
                );
            }
            else
            {
                Console.WriteLine(
                    $"[OnDisconnect] No user found for ConnectionId: {request.ConnectionId}"
                );
            }
