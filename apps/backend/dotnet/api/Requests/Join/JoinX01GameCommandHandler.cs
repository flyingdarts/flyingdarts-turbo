...
) : IRequestHandler<JoinX01GameCommand, APIGatewayProxyResponse>
{
    public async Task<APIGatewayProxyResponse> Handle(JoinX01GameCommand request, CancellationToken cancellationToken)
    {
        ...
        Console.WriteLine($"[JoinX01GameCommandHandler] Updating connection ID for player {playerId}");
        // Update connection ID
        await ConnectionService.UpdateConnectionIdAsync(playerId, request.ConnectionId, cancellationToken);

        Console.WriteLine($"[JoinX01GameCommandHandler] Loading game state for GameId: {request.GameId}");
        // Load game state
        ...
        Console.WriteLine($"[JoinX01GameCommandHandler] Getting metadata for game {request.Game.GameId} and player {playerId}");
        var metadata = await MetadataService.GetMetadataAsync(request.Game.GameId.ToString(), playerId, cancellationToken);

        // Populate metadata as the final step
        socketMessage.Metadata = metadata.ToDictionary();
        Console.WriteLine($"[JoinX01GameCommandHandler] Metadata populated with {metadata.ToDictionary().Count} entries");
        ...
        Console.WriteLine($"[JoinX01GameCommandHandler] Notifying {playerConnectionIds.Length} connections");

        // If its just the owner of the message, don't notify
        if (playerConnectionIds.Any(x => x != socketMessage.ConnectionId))
        {
            var connectionsWithoutMessageOwner = playerConnectionIds.Where(x => x != socketMessage.ConnectionId);

            await NotifyRoomAsync(socketMessage, connectionsWithoutMessageOwner.ToArray(), cancellationToken);
            ...
            var notifiedCount = playerConnectionIds.Any(x => x != socketMessage.ConnectionId)
                ...
                : 0;
            Console.WriteLine($"[JoinX01GameCommandHandler] Notified {notifiedCount} connections (excluding message owner)");
        }
    ...
    private async Task UpdateUserConnectionIdAsync(string playerId, string connectionId, CancellationToken cancellationToken)
    {
        Console.WriteLine($"[UpdateUserConnectionId] Updating connection ID for player {playerId} to {connectionId}");
        ...
        if (existingUser is not null)
        {
            ...
            Console.WriteLine($"[UpdateUserConnectionId] Successfully updated and saved connection ID for user {playerId}");
        }
        else
        {
            Console.WriteLine($"[UpdateUserConnectionId] No existing user found for player {playerId}");
        }
    }

    private async Task AddPlayerToGameAsync(JoinX01GameCommand request, string playerId, CancellationToken cancellationToken)
    {
        Console.WriteLine($"[AddPlayerToGame] Checking if player {playerId} already exists in game");

        // If player is already in the game, do nothing
        if (CachingService.State.Players.Any(x => x.PlayerId == playerId))
        {
            Console.WriteLine($"[AddPlayerToGame] Player {playerId} already exists in game, skipping addition");
            return;
        }
        ...
        Console.WriteLine($"[AddPlayerToGame] Adding new player {playerId} to game {request.GameId}");
        ...
        var player = GamePlayer.Create(long.Parse(request.GameId), playerId, meetingToken);
        await DynamoDbService.WriteGamePlayerAsync(player, cancellationToken);
        Console.WriteLine($"[AddPlayerToGame] Written game player to DynamoDB for player {playerId}");

        CachingService.AddPlayer(player);
        Console.WriteLine($"[AddPlayerToGame] Added player to cache. Total players now: {CachingService.State.Players.Count}");

        var user = await DynamoDbService.ReadUserAsync(playerId, cancellationToken);
        CachingService.AddUser(user);
        Console.WriteLine($"[AddPlayerToGame] Added user to cache. Total users now: {CachingService.State.Users.Count}");

        await CachingService.Save(cancellationToken);
        Console.WriteLine($"[AddPlayerToGame] Successfully added player {playerId} to game {request.GameId}");
    }
    ...
    )
    {
        ...
        Console.WriteLine($"[AddParticipantToMeeting] Getting meeting by ID: {request.Game.MeetingIdentifier}");
        var meeting = await MeetingService.GetByIdAsync(request.Game.MeetingIdentifier, cancellationToken);

        if (meeting is null)
        {
            Console.WriteLine($"[AddParticipantToMeeting] ERROR: Meeting not found for ID {request.Game.MeetingIdentifier}");
            throw new Exception("Meeting not found");
        }

        if (meeting.Id is null)
        {
            Console.WriteLine("[AddParticipantToMeeting] ERROR: Meeting ID is required but was null");
        ...
        Console.WriteLine($"[AddParticipantToMeeting] Adding participant {request.PlayerName} ({playerId}) to meeting {meetingId}");
        var participantToken = await MeetingService.AddParticipantAsync(joinMeetingRequest, cancellationToken);

        if (participantToken is null)
        {
            Console.WriteLine($"[AddParticipantToMeeting] ERROR: Failed to add participant {playerId} to meeting");
    ...
    private async Task StartGameIfReadyAsync(JoinX01GameCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            Console.WriteLine($"[StartGameIfReady] Game not ready to start, need 2 players but have {currentPlayerCount}");
            return;
        }

        Console.WriteLine($"[StartGameIfReady] Starting game {request.Game!.GameId} with 2 players");
        ...
        Console.WriteLine($"[StartGameIfReady] Updated game status to {GameStatus.Started} for game {request.Game.GameId}");
