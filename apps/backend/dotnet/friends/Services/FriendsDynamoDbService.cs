...
public class FriendsDynamoDbService : DynamoDbService, IFriendsDynamoDbService
{
    ...
    )
    {
        ...
        try
        {
            var batch = _dbContext.CreateBatchWrite<FriendRequest>(
    ...
    )
    {
        ...
        try
        {
            ...
            // Save all changes
            var requestBatch = _dbContext.CreateBatchWrite<FriendRequest>(
                _options.Value.ToOperationConfig()
            );
            requestBatch.AddPutItem(friendRequest);

            var relationshipBatch = _dbContext.CreateBatchWrite<FriendRelationship>(
                _options.Value.ToOperationConfig()
            );
    ...
    )
    {
        ...
        try
        {
            var batch = _dbContext.CreateBatchWrite<FriendRelationship>(
                _options.Value.ToOperationConfig()
            );
