...
public interface IFriendsDynamoDbService : IDynamoDbService
{
    ...
    Task<bool> CheckIfAlreadyFriendsAsync(string userId1, string userId2, CancellationToken cancellationToken);
    Task<bool> CheckExistingRequestAsync(string requesterId, string targetUserId, CancellationToken cancellationToken);
    Task<FriendRequest?> GetFriendRequestAsync(string requestId, CancellationToken cancellationToken);
    Task<List<FriendRequest>> GetIncomingFriendRequestsAsync(string userId, CancellationToken cancellationToken);
...
public class FriendsDynamoDbService : DynamoDbService, IFriendsDynamoDbService
{
    ...
    public async Task<bool> CheckUserExistsAsync(string userId, CancellationToken cancellationToken)
    {
        ...
        {
            Console.WriteLine($"[FriendsDynamoDbService] Error checking if user {userId} exists: {ex.Message}");
            return false;
        }
    }

    public async Task<bool> CheckIfAlreadyFriendsAsync(string userId1, string userId2, CancellationToken cancellationToken)
    {
        Console.WriteLine($"[FriendsDynamoDbService] Checking if users are already friends: {userId1} and {userId2}");
        ...
        try
        {
            ...
            Console.WriteLine($"[FriendsDynamoDbService] Users {userId1} and {userId2} are friends: {areFriends}");
            return areFriends;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[FriendsDynamoDbService] Error checking friendship between {userId1} and {userId2}: {ex.Message}");
            return false;
        }
    }

    public async Task<bool> CheckExistingRequestAsync(string requesterId, string targetUserId, CancellationToken cancellationToken)
    {
        Console.WriteLine($"[FriendsDynamoDbService] Checking if friend request exists from {requesterId} to {targetUserId}");

        try
        {
            var queryFilter = new QueryFilter("PK", QueryOperator.Equal, "FRIEND#REQUEST");
            queryFilter.AddCondition("SK", QueryOperator.BeginsWith, $"{targetUserId}#{requesterId}#");

            var queryConfig = new QueryOperationConfig { Filter = queryFilter };

            var results = await _dbContext
                .FromQueryAsync<FriendRequest>(queryConfig, _options.Value.ToOperationConfig())
                .GetRemainingAsync(cancellationToken);

            var hasExistingRequest = results.Any(r => r.Status == FriendRequestStatus.Pending);
            Console.WriteLine($"[FriendsDynamoDbService] Friend request exists from {requesterId} to {targetUserId}: {hasExistingRequest}");
    ...
    public async Task<FriendRequest?> GetFriendRequestAsync(string requestId, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            Console.WriteLine($"[FriendsDynamoDbService] Friend request {requestId} found: {friendRequest != null}");
            return friendRequest;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[FriendsDynamoDbService] Error getting friend request {requestId}: {ex.Message}");
            return null;
        }
    }

    public async Task<List<FriendRequest>> GetIncomingFriendRequestsAsync(string userId, CancellationToken cancellationToken)
    {
        Console.WriteLine($"[FriendsDynamoDbService] Getting incoming friend requests for user: {userId}");
        ...
        try
        {
            ...
            var incomingRequests = results.Where(r => r.Status == FriendRequestStatus.Pending && r.TargetUserId == userId).ToList();

            Console.WriteLine($"[FriendsDynamoDbService] Found {incomingRequests.Count} incoming friend requests for user {userId}");
            return incomingRequests;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[FriendsDynamoDbService] Error getting incoming friend requests for user {userId}: {ex.Message}");
            return new List<FriendRequest>();
        }
    }

    public async Task<List<FriendRequest>> GetOutgoingFriendRequestsAsync(string userId, CancellationToken cancellationToken)
    {
        Console.WriteLine($"[FriendsDynamoDbService] Getting outgoing friend requests for user: {userId}");
        ...
        try
        {
            ...
            var results = await _dbContext
                .FromScanAsync<FriendRequest>(scanConfig, _options.Value.ToOperationConfig())
                .GetRemainingAsync(cancellationToken);

            var outgoingRequests = results.Where(r => r.Status == FriendRequestStatus.Pending && r.RequesterId == userId).ToList();

            Console.WriteLine($"[FriendsDynamoDbService] Found {outgoingRequests.Count} outgoing friend requests for user {userId}");
            return outgoingRequests;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[FriendsDynamoDbService] Error getting outgoing friend requests for user {userId}: {ex.Message}");
            return new List<FriendRequest>();
        }
    }

    public async Task<List<FriendRelationship>> GetUserFriendsAsync(string userId, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            Console.WriteLine($"[FriendsDynamoDbService] Found {friends.Count} friends for user {userId}");
            return friends;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[FriendsDynamoDbService] Error getting friends for user {userId}: {ex.Message}");
            return new List<FriendRelationship>();
        }
    }

    public async Task<FriendRelationship?> GetFriendRelationshipAsync(string userId, string friendId, CancellationToken cancellationToken)
    {
        Console.WriteLine($"[FriendsDynamoDbService] Getting friend relationship between {userId} and {friendId}");
        ...
        {
            Console.WriteLine($"[FriendsDynamoDbService] Error getting friend relationship between {userId} and {friendId}: {ex.Message}");
            return null;
        }
    }
    ...
    public async Task<List<User>> SearchUsersAsync(string searchTerm, string authUserId, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var results = await _dbContext
                .FromScanAsync<User>(scanConfig, _options.Value.ToOperationConfig())
                .GetRemainingAsync(cancellationToken);

            Console.WriteLine($"[FriendsDynamoDbService] Database scan completed. Found {results.Count()} total users");
            ...
            var filteredUsers = results
                .Where(u =>
                    (u.Profile?.UserName?.Contains(searchTerm, StringComparison.OrdinalIgnoreCase) == true)
            ...
            Console.WriteLine($"[FriendsDynamoDbService] Found {filteredUsers.Count} users matching search term '{searchTerm}'");
            return filteredUsers;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[FriendsDynamoDbService] Error searching users with term '{searchTerm}': {ex.Message}");
    ...
    public async Task SaveFriendRequestAsync(FriendRequest friendRequest, CancellationToken cancellationToken)
    {
        Console.WriteLine(
        ...
        try
        {
            var batch = _dbContext.CreateBatchWrite<FriendRequest>(_options.Value.ToOperationConfig());
    ...
    public async Task AcceptFriendRequestAsync(FriendRequest friendRequest, CancellationToken cancellationToken)
    {
        Console.WriteLine(
        ...
        try
        {
            ...
            // Save all changes
            var requestBatch = _dbContext.CreateBatchWrite<FriendRequest>(_options.Value.ToOperationConfig());
            requestBatch.AddPutItem(friendRequest);

            var relationshipBatch = _dbContext.CreateBatchWrite<FriendRelationship>(_options.Value.ToOperationConfig());
    ...
    public async Task DeclineFriendRequestAsync(FriendRequest friendRequest, CancellationToken cancellationToken)
    {
        Console.WriteLine(
    ...
    {
        var relationship1Info = relationship1 != null ? $"{relationship1.RequesterId}-{relationship1.FriendId}" : "null";
        var relationship2Info = relationship2 != null ? $"{relationship2.RequesterId}-{relationship2.FriendId}" : "null";
        Console.WriteLine($"[FriendsDynamoDbService] Deleting friend relationships: {relationship1Info} and {relationship2Info}");

        try
        {
            var batch = _dbContext.CreateBatchWrite<FriendRelationship>(_options.Value.ToOperationConfig());
